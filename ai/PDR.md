...
- **流程**: 用户选择 ETH/USD 市场，使用 ETH 和 USDC 调用 `createDeposit`
- **系统行为**: 
  - Router 接收代币并创建存款请求
  - Keeper 使用预言机价格执行存款
  - 根据池价值计算并铸造相应的 MarketToken LP 份额
  - 用户开始获得该市场的交易费用和借贷费用分成

**LP 份额计算公式**:
```
存款价值 = ETH数量 × ETH价格 + USDC数量
池总价值 = 当前池中ETH价值 + 当前池中USDC价值 + 未实现盈亏
LP份额铸造量 = (存款价值 × 当前LP总供应量) / 池总价值

未实现盈亏计算 (基于 MarketUtils.sol):
netPnl = longPnl + shortPnl
longPnl = (多头持仓token数量 × 当前价格) - 多头总持仓成本
shortPnl = 空头总持仓成本 - (空头持仓token数量 × 当前价格)
最终池价值 = 基础池价值 - netPnl - 影响池价值

说明:
- 多头盈利时 longPnl > 0，意味着池子需要支付给多头交易者
- 空头盈利时 shortPnl > 0，意味着池子需要支付给空头交易者  
- netPnl > 0 表示交易者总体盈利，池价值减少
- netPnl < 0 表示交易者总体亏损，池价值增加
